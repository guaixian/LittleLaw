...
聊天、群聊、语音消息、任意复制粘贴、文件传输、音视频通话,
全部走 TLS 加密 + Protobuf(gRPC)。
...
- **消息类型**:文字、图片(内联缩略/全屏预览)、视频(内联播放)、任意文件、
  **语音消息**(点击录音,AAC 编码,气泡内波形进度播放)、剪贴板一键互传;
- **已读回执**:自己发的消息 ✓ 已送达 / 蓝色 ✓✓ 对方已读,打开会话自动回执,
  离线走 ops 补发,多设备镜像同步;
- **表情回应**:长按消息快捷回应(👍❤️😂😮😢🙏),气泡角标聚合计数,
  单设备单 emoji 增量更新,天然可交换无冲突;
- **消息搜索**:设备页搜索入口,全库文本/文件名 LIKE,点击直跳会话;
- **转发与分享**:长按消息「转发」到任意会话(1:1 或群);「分享到其他应用」
  — Android 拉起系统分享面板(微信/QQ/飞书),Windows 文件/文本入剪贴板
  (桌面版微信/QQ/飞书直接粘贴);
- **收分享**:Android 系统分享面板选 LittleLaw,文本/文件/多文件直达转发选择器;
- **桌面效率**:文件拖进聊天窗直接发送;「粘贴发送」支持截图(CF_DIB→PNG)直发;
- **群聊**:建群(成员多选)、改群名、拉人、踢人、退群、解散,群定义
  (GroupSync)自动扇出 + 离线补发,被移出/解散各端自动清理;
- **群文件多源拉取**:发送者离线时自动改从已下载成功的其他在线成员拉取
  (断点续传保留);
- **多设备镜像**:标记「我的设备」后消息全量镜像,防回环;
- **音视频通话**:1:1 语音/视频(WebRTC DTLS-SRTP),信令走已认证信封通道
  (局域网 gRPC / WebRTC 链路均可),群通话暂未支持。
...
- 聊天页:渐变我方气泡、时间分隔条、长按/右键动作面板(快捷回应/复制/
  转发/分享/多选/双端删除)、多选批量删除、图片内联缩略图与全屏缩放预览、
  视频全屏播放(media_kit)、语音气泡波形进度。

### 备份与恢复

设置页一键打包设备身份 + 配对关系 + 全部聊天记录(zip →
PBKDF2 150k 轮口令派生密钥 → AES-256-GCM,`.llbk` 文件)。
恢复到新设备沿用原身份,好友无需重新配对;口令错误无法解密(无找回)。
...
│              群聊:同msg_id扇出幂等去重+GroupSync群定义同步;                │
│              回执/回应增量op(reaction/receipt);                            │
│              attachExternalTransport: 外部链路(WebRTC)可插拔              │
│  transfer    gRPC拉取式(1MiB chunk) ‖ 信封式(64KiB,WebRTC)             │
│              .part 断点续传 + 整包 SHA-256 校验 + 群文件多源拉取          │
│  rendezvous  可选中转:presence/信令转发/离线邮箱/FCM唤醒/UPnP端点上报    │
│  store       SQLite(messages / ops / peers / groups)                      │
...
| 参数 | 默认 | 说明 |
|---|---|---|
| `-max-conns` | 4096 | 全局并发连接上限 |
| `-conn-burst` | 30 | 每 IP 每分钟新建连接数(令牌桶) |
| `-msg-burst` | 200 | 每连接 10 秒消息数,超限断连 |

固定限额:邮箱每箱 500 封 / 16 MiB,单封密文 ≤256 KiB,全库 2 GiB
(`Ack` 删除即回收);订阅列表 ≤512 设备。箱满会向发送方回明确错误。
...
# 群聊:建群扇出→消息去重→回执→回应→改名→踢人→重拉→解散三端一致
# 群文件多源:发送者离线后从其他成员拉取成功
# 多设备镜像 / rendezvous 邮箱与presence / pair_answer 一扫即成 / 备份编解码
```
...
| 引导包前缀 | `LLB3.`(远程,lzma+base45 分片)/ `LLT1.`(局域网 tap) |
| 分片续传码 | `LLQ2.` |
